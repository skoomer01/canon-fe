...
function SearchByVersionPrivate(props) {
  ...
  const handleSearch = async () => {
    try {
      const response = await SearchApi.getByVersionFromPrivate(
        props.version,
        userId,
        RESULTS_PER_PAGE,
        currentPage
  ...
  const handleCountPages = async () => {
    try {
      const response = await SearchApi.countPagesForVersionFromPrivate(
        props.version,
        userId,
        RESULTS_PER_PAGE
      );
...
export default SearchByVersionPrivate;